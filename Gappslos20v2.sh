...
export PIXELDRAIN_TOKEN="Fb16d150-f848-448c-9891-c65c9d53091a"
...
upload_to_gofile() {
    ...
    if [ -f "$file_path" ]; then
        ...
        if [ -n "$server" ]; then
            echo "🚀 Uploading $(basename "$file_path") to your personal Gofile account..."
            ...
            if [ -n "$download_page" ]; then
                ...
                echo "⚠️ Gofile upload completed but failed to parse folder link: $response"
            fi
        else
...
# Helper function to handle Pixeldrain API uploads
upload_to_pixeldrain() {
    ...
    if [ -f "$file_path" ]; then
        ...
        if [ -n "$PIXELDRAIN_TOKEN" ]; then
            ...
            # Anonymous fallback upload if token is missing
        ...
        if [ -n "$file_id" ]; then
            ...
            echo "⚠️ Pixeldrain extraction failed. Response: $response"
        fi
    fi
}

# Process, Rename, and Dual-Upload Flashable ROM
...
# Process, Rename, and Dual-Upload OTA Update File
