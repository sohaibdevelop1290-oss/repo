...
# 🔄 Base Source: Pre-Modified Manual Trees for PixelOS A15 (bp1a)
...
# ---------------------------------------------------------------------
echo "⚙️ Setting up PixelOS Android 15 environment parameters..."
...
# ---------------------------------------------------------------------
echo "⚙️ Initializing upstream PixelOS Android 15 manifest..."
repo init --depth=1 -u https://github.com/PixelOS-AOSP/manifest.git -b fifteen --git-lfs

# FIXING VENDOR_GMS MISSING BRANCH ISSUE VIA LOCAL MANIFEST
echo "🛠️ Configuring local manifest to bypass broken vendor_gms upstream branch..."
mkdir -p .repo/local_manifests
cat <<EOF > .repo/local_manifests/remove_gms.xml
<?xml version="1.0" encoding="UTF-8"?>
<manifest>
  <!-- Removing the broken upstream vendor_gms definition -->
  <remove-project name="PixelOS-AOSP/vendor_gms" />
</manifest>
EOF
...
# ---------------------------------------------------------------------
echo "📂 Cloning manually modified PixelOS device tree..."
...
# ---------------------------------------------------------------------
# 5. COMPILATION INITIATION (Android 15 Flag-Fix Execution)
# ---------------------------------------------------------------------
echo "🔧 Setting up build environment..."
. build/envsetup.sh

# Exporting explicit Android 15 QPR1 release configs to prevent dumpvars crash
export TARGET_RELEASE=bp1a
export TARGET_PRODUCT=pixelos_billie2
export TARGET_BUILD_VARIANT=userdebug

echo "🚀 Selecting target via explicit Android 15 platform flags..."
...
# ---------------------------------------------------------------------
# 9. INSTANT IMAGE ARREST BLOCK (Converts super_empty.img to ZIP Automatically)
# ---------------------------------------------------------------------
echo "🔒 Triggering immediate target file inspection and capture..."

# Instantly locate super_empty.img and convert it into a protected zip file
...
FLASHABLE_ZIP=$(find "$ROM_DIR" -maxdepth 1 -name "PixelOS_*.zip" -o -name "pixelos_*.zip" | grep -v "ota" | tail -n 1)
OTA_ZIP=$(find "$ROM_DIR" -maxdepth 1 -name "pixelos_billie2-ota-*.zip" -o -name "PixelOS_billie2-ota-*.zip" | tail -n 1)
...
# Execution of Exports (Gofile Exclusive)
